...
%  Fax: +81-45-924-5977.
  ...
  , Nicolas Molina~\authornote[4]{Department of Mechanical and Metallurgical Engineering, Escuela de Ingenieria, Pontificia Universidad Catolica de Chile, Vicuna Mackenna 4860, Santiago, Chile.
...
\end{align}
where the dual variable is updated via dual ascent method. For reference, the algorithm called the method of multipliers (or the augmented Lagrangian method) updates the variables as follows: 
\begin{subequations}
\begin{alignat}{3}
  (\bi{x}^{k+1},\bi{y}^{k+1})
...
\end{array}
\end{align}
where $\epsilon^{abs} > 0$ and $\epsilon^{real} > 0$ is an absolute and relative tolerance, respectively. A reasonable value for the absolute and relative tolerance might be $10^{-6}$ and $10^{-3}$, respectively. This values typically depend on the scale of the data size.
...
\begin{subequations}\label{P.original.friction.SOCP.1}%
  ...
  \end{alignat}
\end{subequations}
where $\bi{s} \in \Re^{m}$ is a parameter vector and $\Phi(\bi{s}) = [\mu^{\alpha} \| E_{t} \bi{s}^{\alpha} \| \hat{e}_{n}, \alpha = 1...n_{c}]^{\top}$. Let $\delta_{K_{e,\mu}^{*}} : \Re^{n} \to \overRe$ denote the 
...
Then problem \eqref{P.original.friction.SOCP.1} is equivalently rewritten as follows:
 \begin{subequations}\label{P.quadratic.SOCP.2}%
   ...
   \end{alignat}
 \end{subequations} 
Therefore, in the notation of \eqref{P.convex.1}: $f(\bi{v}) = \frac{1}{2} \bi{v}^{\top} M \bi{v} + \bi{f}^{\top} \bi{v}, g(\tilde{\bi{u}}) = \delta_{K_{e,\mu}^{*}}(\tilde{\bi{u}}), A = H , B = -I$ and $\bi{c} = -\bi{w} -\Phi(\bi{s})$.
...
\end{align}
moreover, it is recognized in \eqref{eq.ADMM.friction.2.2bi} the fixed-point iteration solution of the VI formulation of \eqref{CF.complementary}, described by Acary in \citep{acary:hal-01630836}, which is related to the zeroes of the natural map, i.e.,
\begin{align}
F_{\rho}^{nat}(\tilde{\bi{u}}) = \tilde{\bi{u}} - \Pi_{K_{e,\mu}^{*}}(\tilde{\bi{u}} - \rho \bi{r}) = 0 \label{eq.ADMM.naturalmap}
\end{align}
where $\bi{r}$ is the contact reaction or impulse (see Appendix B for the relationship between $\bi{r}$ and $\bi{\zeta}$). This formulation helps us to notice the fixed-point interation for the VI formulation immersed in the ADMM algorithm.
...
In Table \ref{Table:2} are depicted the best algorithms with their respective optimal penalty parameter:
...
Although their behavior are very close, Di Cairano takes into account the data size of the problem, which makes it a more general solver.
...
\clearpage
\mbox{}
\subsection{Numerical experience}
...
\begin{figure}[hbtp]
\centering
\includegraphics[width=\textwidth]{Results/S_update/External13.png}
...
\begin{figure}[hbtp]
\centering
\includegraphics[width=\textwidth]{Results/S_update/Internal13.png}
...
\begin{figure}[hbtp]
\centering
\includegraphics[width=\textwidth]{Results/S_update/Internal13outsidestop.png}
...
%Note: I've done a comparison on the projection of u and -xi in their respectives cones, in order to see if they effectively are in the cones through this numerical method. When I use relative error the values of the reactions are so small (more than machine epsilon), that I was almost comparing zero with zero and it gave me apparent errors.
...
%In the case
%\begin{align*}
%K_{e,\mu} \subseteq K_{e,\mu}^{*} \leftrightarrow
%0 \leq 2\theta \leq \frac{\pi}{2} \leftrightarrow
%0 \leq \mu \leq 1
%\end{align*}
%
%\begin{itemize}
%\item Take off $\bi{r} = \bi{0}$
%\begin{align*}
%\bi{u} - \bi{r} = \bi{u} \in K_{e,\mu}^{*}
%\end{align*}
%
%\item Sticking $\bi{u} = \bi{0}$
%\begin{align*}
%\bi{u} + \bi{r} = \bi{r} \in K_{e,\mu} \subseteq K_{e,\mu}^{*} 
%\end{align*}
%
%\item Sliding $\bi{u} \in \partial K_{e,\mu}^{*}$ and $\bi{r} \in \partial K_{e,\mu}$
%\begin{align*}
%\bi{u} + \bi{r} = \bi{u} + \bi{r} \in K_{e,\mu}^{*}
%\end{align*}
%
%\item Therefore, for any case
%\begin{align*}
%\bi{u} + \bi{r} = \bi{u} + \bi{r} \in K_{e,\mu}^{*} \\
%\Pi_{K_{e,\mu}^{*}}(\bi{u} + \bi{r}) = \bi{u} + \bi{r} \\
%\bi{u} + \bi{r} - \Pi_{K_{e,\mu}^{*}}(\bi{u} + \bi{r}) = \bi{0}
%\end{align*}
%\end{itemize}
%
%In the case
%\begin{align*}
%K_{e,\mu}^{*} \subseteq K_{e,\mu} \leftrightarrow
%\frac{\pi}{2} \leq 2\theta \leq \pi \leftrightarrow
%1 \leq \mu \leq \infty
%\end{align*}
%
%\begin{itemize}
%\item Take off $\bi{r} = \bi{0}$
%\begin{align*}
%\bi{u} + \bi{r} = \bi{u} \in K_{e,\mu}^{*} \subseteq K_{e,\mu}
%\end{align*}
%
%\item Sticking $\bi{u} = \bi{0}$
%\begin{align*}
%\bi{u} + \bi{r} = \bi{r} \in K_{e,\mu}^{*} 
%\end{align*}
%
%\item Sliding $\bi{u} \in \partial K_{e,\mu}^{*}$ and $\bi{r} \in \partial K_{e,\mu}$
%\begin{align*}
...
%\begin{align*}
%\bi{u} + \bi{r} = \bi{u} + \bi{r} \in K_{e,\mu} \\
%\Pi_{K_{e,\mu}^{*}}(\bi{u} + \bi{r}) = \bi{0} \\
%\bi{u} + \bi{r} - \Pi_{K_{e,\mu}^{*}}(\bi{u} + \bi{r}) = \bi{u} + \bi{r}
%\end{align*}
%\end{itemize}
...
\mbox{}
\section{Appendix B}
\subsection{Optimality conditions of problem \eqref{P.original.friction.SOCP.1}}
The Lagrangian reads
\begin{align}
  L(\bi{v};\tilde{\bi{u}};\bi{\lambda}) 
  =& \frac{1}{2} \bi{v}^{\top} M \bi{v} + 
  \bi{f}^{\top} \bi{v} + 
  \delta_{K_{e,\mu}^{*}}(\tilde{\bi{u}}) + 
  \bi{\lambda} ( H \bi{v} + \bi{w} + \Phi(\bi{s}) - \tilde{\bi{u}} )
\end{align}
where $\bi{\lambda}$ is the unscaled Lagrange multiplier, i.e., $\rho \bi{\zeta}$. Its gradient (or subgradient) is

\begin{align}
\partial_{\bi{v}} L(\bi{v};\tilde{\bi{u}};\bi{\lambda}) = 
M \bi{v} + \bi{f} + H^{\top} \bi{\lambda} \\
\partial_{\bi{u}} L(\bi{v};\tilde{\bi{u}};\bi{\lambda}) = 
\partial_{\tilde{\bi{u}}} \delta_{K_{e,\mu}^{*}}(\tilde{\bi{u}}) - \bi{\lambda} \\
\partial_{\bi{\lambda}} L(\bi{v};\tilde{\bi{u}};\bi{\lambda}) = H \bi{v} + \bi{w} + \Phi(\bi{s}) - \tilde{\bi{u}}
\end{align}

Therefore, the optimality conditions are
\begin{align}
M \bi{v} + \bi{f} = - H^{\top} \lambda \\
\lambda \in \partial_{\tilde{\bi{u}}} \delta_{K_{e,\mu}^{*}}(\tilde{\bi{u}}) \\
\tilde{\bi{u}} = H \bi{v} + \bi{w} + \Phi(\bi{s})
\end{align}

But, by definition (see \citep{acary:hal-01630836})
\begin{align}
- \bi{r} \in \partial_{\tilde{\bi{u}}} \delta_{K_{e,\mu}^{*}}(\tilde{\bi{u}})
\end{align}

Finally
\begin{align}
\bi{r} = - \bi{\lambda} = - \rho \bi{\zeta}
\end{align}
