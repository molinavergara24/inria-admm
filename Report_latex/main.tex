...
In this section, following \citep{Acary2013, acary2011formulation} we formulate an abstract, algebraic finite-dimensional frictional contact problem. First, the problem is modeled as a complementarity problem over second-cones, and the properties of the latter are discussed. We end by presenting some instances with contact and friction phenomenon that fits our problem description.


We want to discuss possible numerical solution procedures for the following three-dimensional finite-dimensional frictional contact problem and some of its variants. Let $n_{c} \in \mathbb{N}$ be the number of contact points and $n \in \mathbb{N}$ the number of degrees of freedom of a discrete mechanical system.
...
where the set $K_{e,\mu} \subseteq \Re^{m}$ is the cartesian product of Couloumb's friction second-order cone at each contact, that is
\begin{align}
	K_{e,\mu} = \prod_{\alpha = 1...n_{c}} K_{e,\mu}^{\alpha} =
	\prod_{\alpha = 1...n_{c}} \{ (x_{1},\bi{x}_{2}) \in \Re \times \Re^{n-1} \mid \| \bi{x}_{2} \| \le \mu x_{1} \}
\end{align}

and $K_{e,\mu}^{*} \subseteq \Re^{n}$ denotes the dual cone of $K_{e,\mu}$. The function $\Phi: \Re^{m} \rightarrow \Re^{m}$ is a nonsmooth function defined as
\begin{align}
	\Phi(\bi{u}) = [\mu^{\alpha} \| E_{t} \bi{u}^{\alpha} \| \hat{\bi{e}}_{t}, \alpha = 1...n_{c}]^{\top}
\end{align}
where $\hat{\bi{e}}_{t} = [0, 1, 1]^{\top}$ is the tangential component vector and $E_{t} = diag(0,1,1)$ is a matrix $3 \times 3$.
...
and $\bi{u}$ is called the scaled dual variable. 
...
\clearpage
\mbox{}
\section{ADMM formulation of problem \eqref{P.original.friction.SOCP.1}}
\label{sec:friction}
...
\begin{subequations}\label{P.original.friction.SOCP.1}%
  ...
  \end{alignat}
\end{subequations}
where $\bi{s}$ is a parameter vector and $K_{e,\mu}^{*}$ is defined by 
\begin{align}
  \ K_{e,\mu}^{*} = \{ (x_{1},\bi{x}_{2}) \in \Re \times \Re^{n-1}
...
\begin{align}
  ...
  \end{dcases*}
\end{align}
Then problem \eqref{P.original.friction.SOCP.1} is equivalently rewritten as 
follows: 
\eqref{P.original.friction.SOCP.1} as follows: 
 \begin{subequations}\label{P.quadratic.SOCP.2}%
   \begin{alignat}{3}
     & \MIN  &{\quad}& 
    \frac{1}{2} \bi{v}^{\top} M \bi{v} + \bi{f}^{\top} \bi{v} + \delta_{K_{e,\mu}^{*}}(\hat{\bi{u}}) \\
     & \ST & & 
     \tilde{\bi{u}} = H \bi{v} + \bi{w} + \Phi(\bi{s})   
   \end{alignat}
 \end{subequations} 
where $\Phi(\bi{s}) = [\mu^{\alpha} \| E_{t} \bi{s}^{\alpha} \| \hat{e}_{t}, \alpha = 1...n_{c}]^{\top}$. Therefore, in this notation $f(\bi{v}) = \frac{1}{2} \bi{v}^{\top} M \bi{v} + \bi{f}^{\top} \bi{v}, g(\tilde{\bi{u}}) = \delta_{K_{e,\mu}^{*}}(\tilde{\bi{u}}), A = H , B = -I$ and $\bi{c} = -\bi{w} -\Phi(\bi{s})$.
...
\end{align}
where $\rho$ is the penalty parameter, and $\bi{\zeta} \in \Re^{n}$ is  the scaled dual variable (the scaled Lagrange multiplier). 
...
A key ingredient in the efficiency and the convergence of ADMM is the choise of the sequence $\lbrace \rho_{k} \rbrace$. A sensible work has been done in the literature mainly motivated by the rate of convergence. This section presents the most popular approach for choosing the sequence $\lbrace \rho_{k} \rbrace$ for QPs.

\subsection{Optimal penalty parameter selection for QP}
The ADMM technique is surprisingly robust to poorly selected algorithm parameters: under mild conditions, the method is guaranteed to convergence for all positive values of $\rho$ \citep{boyd2011distributed, deng2016global, eckstein2012augmented}. However, the convergence time is heavily affected by the choice of the algorithm parameter.

Two main results have been proposed for QPs in the initial guess of the parameter:

\subsubsection{Ghadimi}
The $\rho^{\bigstar}$ that minimizes the rate of convergence \citep{ghadimi2015optimal} is
\begin{align}
  \rho^{\bigstar} := \left(\sqrt{\lambda_{min_{\neq 0}}(H M^{-1} H^{T}) \lambda_{max}(H M^{-1} H^{T})}\right) ^{-1} 
\end{align}
If the constraint matrix $H$ is not full rank, the smallest non-zero eigenvalue is taken, which still works as a heuristic to reduce the rate of convergence [Ghadimi].

\subsubsection{Di Cairano}
The $\rho^{\bigstar}$ that minimizes the rate of convergence \citep{raghunathan2014alternating} is
\begin{align}
  \rho^{\bigstar} := \sqrt{\lambda_{min}(M) \lambda_{max}(M)}
\end{align}
In this case, the nullity of H equal to zero is avoided.

\subsubsection{Acary}
In this report is propposed the following value, where the penalty parameter could be seen as the factor that balance the matrix involved in the the first term of \eqref{eq.ADMM.friction.2.1}
\begin{align}
  \rho^{\bigstar} := \frac{\Vert M \Vert_{1}}{\Vert H \Vert_{1}}
\end{align}

\subsubsection{Normal}
A less expensive and standard method is to set
\begin{align}
  \rho^{\bigstar} := 1
\end{align}
...
\end{array}
\end{align}
This approach avoids the need for explicit compensation for problem scaling when the formulation is modified. The stopping critera in (eq.) is invariant to problem scaling when $\epsilon^{abs} = 0$.
...
\end{array}
\end{align}
where SD stands for \textit{steepest descent} and MG for \textit{minimum gradient}. A hybrid stepsize rule is proposed
\begin{align}
\begin{array}{l}
  \hat{\alpha}_{k} := 
  \begin{dcases*}
...
updates of $\bi{y}^{k}$ and $\bi{w}^{k}$ which takes into account past iterations. It is shown that the method converges in the primal and dual residuals with rate $O(1/k^{2})$.
\begin{align}
    \alpha_{k+1} := \frac{1}{2} \Bigl( 1 + \sqrt{1 + 4\alpha_{k}^{2}} \Bigr) 
    \label{eq.ADMM.relaxed.1.1} \\
    \hat{\bi{y}}^{k+1}
    := \bi{y}^{k+1} + \frac{\alpha_{k}-1}{\alpha_{k+1}} (\bi{y}^{k+1}-\bi{y}^{k}) 
    \label{eq.ADMM.relaxed.1.2} \\
    \hat{\bi{z}}^{k+1}
    := \bi{z}^{k+1} + \frac{\alpha_{k}-1}{\alpha_{k+1}} (\bi{z}^{k+1}-\frac{\rho^{k-1}}{\rho^{k}} \bi{z}^{k})
    \label{eq.ADMM.relaxed.1.3}
\end{align}
...
\end{align}
if $e_{k} < \eta e_{k-1}$ the relaxed ADMM \eqref{eq.ADMM.relaxed.1.1}, \eqref{eq.ADMM.relaxed.1.2} and \eqref{eq.ADMM.relaxed.1.3} is performed, otherwise, the restart rule states
\begin{align} 
    \alpha_{k+1} := 1 \\
    \hat{\bi{y}}^{k+1}
    := \bi{y}^{k+1} \\
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \State
    $\bi{x}^{k+1} 
    := \argmin_{\bi{x}} L_{\rho}(\bi{x};\hat{\bi{y}}^{k};{\bi{z}}^{k})$ 
    \State
    $\bi{y}^{k+1} 
    := \argmin_{\bi{y}} L_{\rho}(\bi{x}^{k+1};\bi{y};{\bi{z}}^{k})$ 
    \State
...
\mbox{}
\subsection{Results}
In this section, we perform a comparison of the algorithms by optimal penalty parameter. The goal is to study the influence of the various parameters and possible strategies on the performance of the solvers. 
...
\mbox{}
\subsubsection{Di Cairano}
Figure 2 shows that most of the algorithms suffer of robustness. Wohlberg and Spectral $\rho$-updates depict the worst behavior. Furthermore, the difference bewteen the constant and varying penalty parameter is highly comparable in He $\rho$-update.
...
\mbox{}
\subsubsection{Acary}
Figure 2 shows that most of the algorithms suffer of robustness. Wohlberg and Spectral $\rho$-updates depict the worst behavior. Furthermore, the difference bewteen the constant and varying penalty parameter is only comparable in He $\rho$-update.
...
In Table \ref{Table:2} are despicted the best algorithms with their respective optimal penalty parameter. Although their behavior are very close, Di Cairano takes into account the data size of the problem, which makes it a more general solver.
...
Another issue is the initial guess of $s$ to improve the rate of convergence. One heuristic method that performs well in our tests is
\begin{align}
	\Phi^{0}(\bi{s}) = \Phi(\bi{s}^{0}) := \frac{1}{\| H \|_{F}}  \frac{\Phi(\mathbf{1})}{\| \Phi(\mathbf{1}) \|}
\end{align}
...
\begin{align}
	\Phi(\bi{u})
	= [\mu^{\alpha} \| E_{t} \bi{u}^{\alpha} \| \hat{\bi{e}}_{t}, \alpha = 1...n_{c}]^{T}
...
In Figure 7 is despicted the same internal update, but the stop criterion \eqref{eq.stopcriterion} is changed for the external one  \eqref{eq.stopcriterionexternal}. It is observed that $\| \Phi(s) \|$ holds from iteration 25, but $\Phi(s)^{\alpha}$ is not the same from iteration to iteration, the test was stopped at 1000 iterations.
\begin{figure}[hbtp]
\centering
