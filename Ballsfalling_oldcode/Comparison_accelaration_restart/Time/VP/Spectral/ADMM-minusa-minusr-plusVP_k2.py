...
for k in range(MAXITER):
	...
	if k % 2 == 0:
		...
		alfa_SD = np.absolute(np.nan_to_num(Dlambda_dot / DH_Dlambda_dot))
		alfa_MG = np.absolute(np.nan_to_num(DH_Dlambda_dot / DH_dot))

		beta_SD = np.absolute(np.nan_to_num(Dlambda_dot / DG_Dlambda_dot))
		beta_MG = np.absolute(np.nan_to_num(DG_Dlambda_dot / DG_dot))
		...
		#Correlations
		alfa_corr = np.absolute(np.nan_to_num(DH_Dlambda_dot / (DH_norm * Dlambda_norm)))
		beta_corr = np.absolute(np.nan_to_num(DG_Dlambda_dot / (DG_norm * Dlambda_norm)))
		...
		print alfa_SD,alfa_MG,alfa_corr,beta_SD,beta_MG,beta_corr
