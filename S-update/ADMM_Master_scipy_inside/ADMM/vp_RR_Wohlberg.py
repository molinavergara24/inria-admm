...
def vp_RR_Wohlberg(problem_data, rho_method):
	...
	for k in range(MAXITER):
		...
		eps_pri_scaling = np.sqrt(p)*ABSTOL/pri_evalf + RELTOL
		eps_dual_scaling = np.sqrt(n)*ABSTOL/pri_evalf + RELTOL		

		r_normW.append(np.linalg.norm(r[k+1] / pri_evalf))
		s_normW.append(np.linalg.norm(s[k+1] / dual_evalf))
		if r_normW[k+1] <= eps_pri_scaling + RELTOL and s_normW[k+1] <= eps_dual_scaling:
			break
		...
		################################
		rho.append(penalty(rho[k], r_normW[k+1], s_normW[k+1], mu))	
